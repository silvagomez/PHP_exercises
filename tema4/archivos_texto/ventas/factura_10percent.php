...
echo <<<abc
    <h1>FACTURAS 10% DE LOS CLIENTES</h1>
abc;
$linea_cliente=fgets($archivo_cliente);
$total_clientes=0;
while (!feof($archivo_cliente)) { // recorro el documento para saber cuantos clientes hay
    $linea_cliente=fgets($archivo_cliente);
    $total_clientes++;
}
...
$fecha=date('j/n/Y');
$total=0;
while ($contador_porciento<$diez_porciento_cliente) { // pregunta sí no es fin de fichero
    $linea_cliente=fgets($archivo_cliente);
